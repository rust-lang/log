...
//! A lightweight logging facade, no_std version.
//!
...
//! configuration.
//!
//! Unlike the `log` crate, `log_core` only depends on the Rust core library,
//! which allows it to be used in environments that do not have support for
//! memory allocation.
//!
//! # Use
//!
//! ## In libraries
//!
//! Libraries should link only to the `log_core` crate, and use the provided
...
//!
//! The executable itself may use the `log_core` crate to log as well.
//!
//! ### Warning
//!
...
//! extern crate log_core;
//!
//! use log_core::{LogRecord, LogLevel, LogMetadata};
//!
//! struct SimpleLogger;
//!
//! impl log_core::Log for SimpleLogger {
...
//! ```
//!
//! Loggers are installed by calling the `set_logger_raw` function and can be
//! later removed by calling the `shutdown_logger` function. `set_logger_raw`
//! takes a closure which is provided a `MaxLogLevel` token and returns a `Log`
//! trait object. The `MaxLogLevel` token controls the global maximum log level.
//! The logging facade uses this as an optimization to improve performance of
//! log messages at levels that are disabled. In the case of our example logger,
...
//! ```rust
//! # extern crate log_core;
//! # use log_core::{LogLevel, LogLevelFilter, SetLoggerError,
//! #                ShutdownLoggerError, LogMetadata};
//! # struct SimpleLogger;
//! # impl log_core::Log for SimpleLogger {
//! #   fn enabled(&self, _: &LogMetadata) -> bool { false }
//! #   fn log(&self, _: &log_core::LogRecord) {}
//! # }
//! # impl SimpleLogger {
//! #   fn flush(&self) {}
//! # }
...
//!     unsafe {
//!         log_core::set_logger_raw(|max_log_level| {
...
//!     log_core::shutdown_logger().map(|logger| {
...
// The setup here is a bit weird to make shutdown_logger work.
//
...
// shut down after calling shutdown_logger.
//
// The LOGGER static holds a pointer to the global logger. It is protected by
// the STATE static which determines whether LOGGER has been initialized yet.
//
// The shutdown_logger routine needs to make sure that no threads are actively
// logging before it returns. The number of actively logging threads is tracked
// in the REFCOUNT static. The routine first sets STATE back to INITIALIZING.
...
// increment and decrement it, but the interval in between is  small enough
// that the wait is really just for the active log calls to finish.
...
/// duration of the program or until `shutdown_logger` is called.
...
/// events that occur after the call to `shutdown_logger` completes will be
/// ignored.
///
...
/// concurrently the logger object.
...
/// The type returned by `set_logger_raw` if `set_logger_raw` has already been
/// called.
#[allow(missing_copy_implementations)]
#[derive(Debug)]
...
impl error::Error for SetLoggerError {
    fn description(&self) -> &str { "set_logger_raw() called multiple times" }
}

/// The type returned by `shutdown_logger` if `shutdown_logger` has already been
/// called or if `set_logger_raw` has not been called yet.
#[allow(missing_copy_implementations)]
#[derive(Debug)]
...
mod tests {
     ...
     fn test_error_trait() {
         ...
         assert_eq!(e.description(), "set_logger_raw() called multiple times");
     }
}
